...
content_data = {
    labels[0]: {
        ...
        'texts': [
            "화농성 여드름의 주요 원인은 피지과다 미생물 번식 호르몬 변화가 있습니다",
            "화농성 여드름을 가라앉히는 방법에는 세안하기,약물치료,여드름 패치,청소하기,자외선 차단하기 등이 도움이 될 수 있습니다."
        ]
    },
    labels[1]: {
        'images': [
        ...
        'texts': [
            "낭포성 여드름은 농포가 밖으로 터져 나오지 못하고 안에서 누적되어 결절 형태를 이루는 상태를 말합니다.이는 주로 피부유형과 더불어 호르몬 변화 스트레스,영양불균형,또는 잘못된 피부관리로 인해 발생할 수 있습니",
            "낭포성 여드름을 가라앉히는 방법에는 부드럽고 순한 클렌징 제품을 사용하여 적절한 피부관리가 필요하고,지속적인 보습,식습관 관리,약물치료,레이저 치료등이 도움이 될수 있습니다."
        ]
    },
    labels[2]: {
        'images': [
        ...
        'texts': [
            "결절성 여드름은 여드름진행 단계중 3단계에 해당하는 중증도 여드름으로, 구진과 농포를 거쳐 염증반응이 지속됨에 따라 정상적이 모낭 내벽의 형태가 파괴되고 여러개의 염증이 합쳐져 피부속에 더 크고 깊게 자리하는 것이 특성입니.",
            "결절성 여드름을 가라앉히는 방법에는 염증주사,식습관 개선,예를 들어 맵고 자극적인 음식을 먹는 것을 피하는 것 그리고 클렌징을 통해 세안을 해주는 것이 도움이 될 수 있습니다."
        ]
    }
}
